...
async def test_mcp_schema_snapshot(server_name, mcp_server, update_snapshots):
    ...
    assert snapshot_path.exists(), (
        f"No snapshot for {server_name}. Run: uv run pytest tests/test_tool_schemas.py --update-snapshots"
    )

    expected = json.loads(snapshot_path.read_text())
    assert tools == expected, (
        f"{server_name} schema changed! "
        f"Run: uv run pytest tests/test_tool_schemas.py --update-snapshots"
    )
